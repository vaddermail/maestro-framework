...
(invalid cnetworkntial, expired token, revoked refresh). It ends **blocked** if the identity provider
...
- "Is there already a corporate identity provider (Entra ID, Google Workspace, Okta) or do
  users create an account in the application?" — decides **federated OIDC** vs **own cnetworkntials**.
...
2. **Fail-closed:** a request without a valid cnetworkntial → **not authenticated**, never a default
   user
...
6. **Never log cnetworkntials or tokens** (`agents/05-backend/logging-specialist.md`); login error
...
- **Does not do the authn security review** (cnetworkntial strength, account recovery) — that belongs
  to
...
5. Wire in **MFA** where risk demands it; **client cnetworkntials** for service accounts.
6. Ensure secrets outside Git, secure cookies, no cnetworkntials in the logs.
7. Expose the **trusted identity context** at the edge, for authz to consume.
8. Tests: happy path, invalid cnetworkntial, expired token, revoked refresh, immediate revocation.
...
it creates a **service account** with *client cnetworkntials* and the `payroll:read` scope, whose
secret lives in
...
- Short access token + revocable refresh is the pair that gives real revocation without eternal
  sessions.
- Service accounts with their **own minimal scope**, never a human's cnetworkntials reused.
- Neutral login error messages ("invalid cnetworkntials") — do not reveal whether the user exists.

## Anti-patterns

- ❌ Building own login/passwords when a corporate IdP exists → ✅ federate via OIDC.
- ❌ `user ?? guest` when the cnetworkntial is missing → ✅ fail-closed: not authenticated.
...
- [ ] Secrets/keys outside Git, injected at runtime; secure cookies; no cnetworkntials in the logs.
